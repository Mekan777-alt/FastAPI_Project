...
async def get_notification(notification_id: int, user: Annotated[dict, Depends(get_firebase_user_from_token)],
                           session: AsyncSession = Depends(get_session)):
    try:

        notification = await session.scalar(select(NotificationTenants)
                                            .where(NotificationTenants.id == notification_id))
